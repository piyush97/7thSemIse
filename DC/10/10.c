...
int main(int argc, char *argv[])
{
  ...
  {
    printf("\n");
    printf("COMMUNICATOR_MPI: Fatal error!\n");
    printf("  MPI_Init returned nonzero error flag\n");
    exit(1);
  }
  ...
  /*Try to get ID of each process in both groups.If a process is not in a communicator, set its ID to -1.*/
  if (id % 2 == 0)
  {
  ...
  /*Use MPI_Reduce to sum the global ID of each process in the even group.Assuming 4 processes: EVEN_SUM = 0 + 2 = 2;*/
  if (even_id != -1)
  {
  ...
  /*Use MPI_Reduce to sum the global ID of each process in the odd group.Assuming 4 processes: ODD_SUM = 1 + 3 = 4;*/
  if (odd_id != -1)
  {
